...
import ATMClass from '../class/ATMClass.js';
import MessageHandlerClass from '../class/MessageHandlerClass.js';
import handleInput from '../inputHandling.js';
...
describe('handleInput', function() {
  ...
  describe('login function', function() {
    ...
    it('should not log in the user if they are already logged in', function() {
      ...
      assert.strictEqual(consoleLogStub.getCall(1).args[0], messageHandler.otherInfo(90001));
    });

    it('should prompt the user to enter a name if no name is provided', function() {
      handleInput('login');
      assert.strictEqual(consoleLogStub.getCall(0).args[0], messageHandler.otherInfo(90002));
    });
  });
  ...
  describe('deposit function', function() {
    ...
    it('should not deposit any amount if the user is not logged in', function() {
      handleInput('deposit 100');
      assert.strictEqual(consoleLogStub.getCall(0).args[0], messageHandler.otherInfo(90003));
    });

    it('should prompt the user to enter an amount if no amount is provided', function() {
      handleInput('login Alice');
      handleInput('deposit');
      assert.strictEqual(consoleLogStub.getCall(1).args[0], messageHandler.otherInfo(90004));
      handleInput('logout');
    });
  });
  ...
  describe('withdraw function', function() {
    ...
    it('should withdraw the given amount if the user is logged in and has enough balance', function() {
      const alice = new ATMClass('Alice');
      alice.balance = 200;
      handleInput('login Alice');
    ...
    it('should not withdraw any amount if the user is not logged in', function() {
      handleInput('withdraw 100');
      assert.strictEqual(consoleLogStub.getCall(0).args[0], messageHandler.otherInfo(90005));
    });
  ...
  describe('transfer function', function(){
    ...
    it('should not transfer any amount if the user is not logged in', function() {
      ...
      assert.strictEqual(consoleLogStub.getCall(0).args[0], messageHandler.otherInfo(90006));
    });

    it('should prompt the user to enter an amount if no amount is provided', function() {
      handleInput('login Alice');
      handleInput('transfer');
      assert.strictEqual(consoleLogStub.getCall(1).args[0], messageHandler.otherInfo(90007));
      handleInput('logout');
    });
    ...
    it('should transfer the given amount to other user if user is logged in and has enough balance', function() {
      handleInput('login Bob');
      handleInput('deposit 100');
      const targetUser = new ATMClass('Alice');
  ...
  describe('Test scenario and commands', function() {
    ...
    it("Alice transferred $30 to Bob, which reduced Bob's debt to $10.", function() {
      ...
      const targetUser = new ATMClass('Bob');
